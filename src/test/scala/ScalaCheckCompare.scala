...
object ScalaCheckCompare extends Properties("Scalacheck - Compare with apache common-csv") {
  ...
  val csvFormats = List((IttoCSVFormat.default.withQuoteEmpty(true), CSVFormat.DEFAULT), (IttoCSVFormat.tab.withQuoteEmpty(true), CSVFormat.TDF))
  val delimiters = List(COMMA, SEMICOLON, PIPE)
  val recordSeparators = List(LF, CRLF)
  val quotes = List(PIPE, DOUBLE_QUOTE)

  def stringToCsvFieldTest(p: IttoCSVFormat, f: CSVFormat, gen: Gen[String]): Prop = forAll(gen) {
    l => StringToCsvField.stringToCsvField(l)(p) == ApacheCommonCsvHelper.fildParser(l)(f)
  }
  ...
  for {
    ...
    quote <- quotes
    if quote != delimiter
  } yield property("stringToCsvFieldTest") = stringToCsvFieldTest(format1.withDelimiter(delimiter).withRecordSeparator(recordSeparator).withQuote(quote),
    format2.withDelimiter(delimiter).withRecordSeparator(recordSeparator).withQuote(quote), generator)
}
